    ...
    return(
        <div className='persona w-full py-2 px-3  rounded-2xl'>
            ...
            <div className='personagens my-5 flex items-stretch  space-x-2'>   
                <input id='nome' type='text' value={selectedName} onChange={handleNameChange} placeholder='Nome do Personagem' maxLength={12} className="shadow appearance-none border rounded-xl py-2 px-3 text-[#727171] leading-tight focus:outline-none focus:shadow-outline flex-grow" ></input>
                {  (<select id='voz' value={selectedVoice} onChange={handleSelectChange} className='shadow aparence-none text-[#727171] rounded-xl p-2 border leading-tight focus:outline-none focus:shadow-outline'>
                    <option value="" disabled>Escolha uma voz</option>
                    {availableVoices.map((voice, index) => (
                        <option key={index} value={voice}>{voice}</option>
                    ))}
                </select>)}
                <button type='button' onClick={handleConfirm} disabled={!selectedVoice || !selectedName} className='bg-[#8CEAFF] text-[#727171] rounded-xl min-h-[10px] p-2 flex-grow'>Adicionar</button>
            </div>
                    ...
                    <div className='bg-white p-2 rounded-lg flex flex-row'>
                        {confirmedVoices.map((item, index) => (
                            <div key={index} className='flex flex-row items-center justify-between mr-2 mb-2'>
                                <button onClick={() => handleRevert(item.voice)} className={`${voiceColors[item.voice]} flex flex-row min-h-[10px] py-1 px-2 pl-2 items-center rounded flex-grow`}>
