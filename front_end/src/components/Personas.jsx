...
const Persona = () => {
    ...
    return (
        <div className='persona'>
            <div className="topo">Personagens</div>
            <div className='personagens space-x-2'>
                <input id='nome' type='text' value={selectedName} onChange={handleNameChange} placeholder='Nome do Personagem' maxLength={12} ></input>
                {(<select id='voz' value={selectedVoice} onChange={handleSelectChange}>
                    <option value="" disabled>Escolha uma voz</option>
                    {availableVoices.map((voice, index) => (
                        <option key={index} value={voice}>{voice}</option>
                    ))}
                </select>)}
                <button type='button' onClick={handleConfirm} disabled={!selectedVoice || !selectedName}>Adicionar</button>
            </div>
                ...
                {confirmedVoices.length > 0 && (
                    <div className='confirmados'>
                        {confirmedVoices.map((item, index) => (
                            <div key={index} className='voz'>
                                <button onClick={() => handleRevert(item.voice)} className={`${voiceColors[item.voice]}`}>
