...
const PraticarRoteiro = () => {
  ...
  const [transcript, setTranscript] = useState(""); // Novo estado para armazenar o transcript
  ...
  useEffect(() => {
    if ("webkitSpeechRecognition" in window) {
      ...
      recognitionInstance.onresult = (event) => {
        ...
        setTranscript(transcript); // Atualiza o estado com o transcript
  ...
  const handlePlayAudio = (dialogue) => {
    ...
    const voiceName =
      characterVoiceMap[dialogue.characterId] || "Google Português do Brasil";
                  ...
                  .map((dialogue, index) => (
                    <div
                      ...
                      className={`mb-4 p-6 rounded-2xl shadow-lg ${
                        characterColors[dialogue.characterId] ||
                        "bg-gradient-to-r from-blue-300 to-blue-500"
                      } relative`}
                    >
                      <p>
                        <strong>Personagem {dialogue.characterId}:</strong>
                      </p>
            ...
            <button
              onClick={() =>
                setCurrentDialogueIndex((index) =>
                  Math.min(index + 1, script.dialoguesList.length - 1)
                )
              }
              disabled={currentDialogueIndex >= script.dialoguesList.length - 1}
              className={`w-1/2 bg-[#00FF62] text-white py-2 px-4 rounded-lg mt-4 ${
                currentDialogueIndex >= script.dialoguesList.length - 1
                  ? "opacity-50 cursor-not-allowed"
                  : "cursor-pointer"
              } hover:bg-[#00ff62d1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500`}
            >
              Continuar
            </button>
          </div>
